    ...
    return (
        <Card>
            <CardContent className="p-6">
                ...
                {/* Total Amount */}
                <div className="mb-4">
                    <div className="mb-1 text-xs text-gray-500">Total Amount</div>
                    <Input value={`₱${totalPaymentAmount.toFixed(2)}`} readOnly className="bg-blue-100 font-bold text-blue-900" />
                </div>

                {/* Subtotal Needed to Pay */}
                <div className="mb-4">
                    <div className="mb-1 text-xs text-gray-500">Subtotal Needed to Pay</div>
                    <Input value={`₱${subtotal.toFixed(2)}`} readOnly className="bg-gray-100 font-bold text-gray-900" />
                </div>

                {/* Payment Difference */}
                <div className="mb-4">
                    <div className="mb-1 text-xs text-gray-500">{paymentDifference >= 0 ? 'Change' : 'Balance Due'}</div>
                    <Input
                        value={`₱${Math.abs(paymentDifference).toFixed(2)}`}
                        readOnly
                        className={`font-bold ${paymentDifference >= 0 ? 'bg-green-100 text-green-900' : 'bg-red-100 text-red-900'}`}
                    />
                </div>
                <div className="mb-4">
                ...
                <Button className="w-full bg-green-900 text-sm font-bold text-white">Settle Payment</Button>
            </CardContent>
        </Card>
    );
}
