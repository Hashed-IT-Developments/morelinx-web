import { Table } from "@/components/composables/table"
...
export default function AmendmentHistory(application:CustomerApplication) {
    ...
    useEffect(() => {
        axios.get(route('customer-applications.amendment-history', application.id))
            .then(response => {
    ...
    return (
        <main>
            <section className="rounded-xl border border-gray-100 p-2">
                <h2 className="text-2xl mt-4">Amendment History</h2>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-3">

                    {amendments.map((item) => {
                        return (
                            <div className={`p-4 border rounded-lg ${getCardBG(item)} shadow`} key={item.id}>
                                <div className="flex items-start justify-between">
                                    <div>
                                        <div>Requested by <strong>{item.user.name}</strong></div>
                                        <div className="text-sm italic text-gray-500">On
                                            {(() => {
                                ...
                                <h4 className="text-lg py-2">Amendment Details</h4>

                                <div className="overflow-x-auto mt-2">
                                    <table className="min-w-full text-sm">
                                        <thead>
                                            <tr>
                                                <th className="text-start px-2 py-2 bg-green-300">Field</th>
                                                <th className="text-start px-2 py-2 bg-green-300">Original Data</th>
                                                <th className="text-start px-2 py-2 bg-green-300">New Data</th>
                                            </tr>
                                        </thead>
                                        <tbody>
